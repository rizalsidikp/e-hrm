        ...
        <div class="container-fluid py-4">
            <div class="card">
                ...
                <div class="card-body pt-4 p-3">
                    <form id="bonus-create" action="/bonus-management" method="POST" role="form text-left">
                        @csrf
                        @if ($errors->any())
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input class="form-control" type="number" id="user.bonus" name="bonus"
                                        placeholder="500000" value="{{ old('bonus') }}" required>
                                    @error('bonus')
                                        <p class="text-danger text-xs mt-2">{{ $message }}</p>
                                    @enderror
                                </div>
                                    ...
                                    <select class="form-control cursor-pointer" id="bonus.bulan" name="bulan" required>
                                        @foreach ($months as $key => $month)
                                            <option value="{{ $month['value'] }}"
                                                {{ (int) old('month', $currentMonth) === $month['value'] ? 'selected' : '' }}>
                                                {{ $month['option'] }}</option>
                                        @endforeach
                                    </select>
                                    @error('bulan')
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control cursor-pointer" id="bonus.tahun" name="tahun" required>
                                        @foreach ($years as $key => $year)
                                            <option value="{{ $year }}"
                                                {{ (int) old('year', $currentYear) === $year ? 'selected' : '' }}>
                                                {{ $year }}</option>
                                        @endforeach
                                    </select>
                                    @error('tahun')
