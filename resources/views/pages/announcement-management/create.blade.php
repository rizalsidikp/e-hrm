...
@section('content')
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <input id="announcement.banner" name="banner" type="hidden" />
                                    @error('banner')
                                        <p class="text-danger text-xs mt-2">{{ $message }}</p>
                                    @enderror
                                </div>
    ...
    @php
        ...
        $data = [
            ...
            'icon' => 'fa-newspaper',
            'url' => 'announcement-management/create',
        ];
...
@section('page-content')
    ...
    <script type="text/javascript">
         ...
         var myDropzone = new Dropzone(drop, {
             ...
             maxFiles: 1,
             init: function() {
                 var bannerInput = document.getElementById('announcement.banner');
                 this.on("complete", function(file) {
                     if (this.getUploadingFiles().length === 0 && this.getQueuedFiles().length === 0) {
                         if (file.xhr && file.xhr.response) {
                             var response = JSON.parse(file.xhr.response);
                             if (response.banner) {
                                 bannerInput.value = response.banner;
                             }
                         }
                     }
                 });
